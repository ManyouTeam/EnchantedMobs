...
public class TextUtil {
    ...
    public static String pluginPrefix() {
        if (!CommonUtil.getMajorVersion(16)) {
            return "§a[EnchantedMobs]";
        }
        return "§x§9§8§F§B§9§8[EnchantedMobs]";
    }
    ...
    public static void sendMessage(Player player, String rawText) {
        ...
        if (!rawText.contains("[")) {
            EnchantedMobs.methodUtil.sendChat(player, rawText);
            return;
        }
        ...
        for (String msg : parseSimpleTag(rawText, "message")) {
            EnchantedMobs.methodUtil.sendChat(player, msg);
            sentAny = true;
        }
        ...
        for (TagResult tag : parseArgTag(rawText, "title")) {
            ...
            EnchantedMobs.methodUtil.sendTitle(
                    player,
                    data.title,
                    data.subTitle,
                    data.fadeIn,
        ...
        for (String msg : parseSimpleTag(rawText, "actionbar")) {
            EnchantedMobs.methodUtil.sendActionBar(player, msg);
            sentAny = true;
        }
        ...
        for (TagResult tag : parseArgTag(rawText, "bossbar")) {
            ...
            EnchantedMobs.methodUtil.sendBossBar(
                    player,
                    data.title,
                    data.progress,
                    data.color,
