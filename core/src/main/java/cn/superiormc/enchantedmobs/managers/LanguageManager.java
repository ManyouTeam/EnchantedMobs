...
public class LanguageManager {
    ...
    private void initLanguages() {
        File langFolder = new File(EnchantedMobs.instance.getDataFolder(), "languages");
        if (!langFolder.exists()) {
            langFolder.mkdirs();
        }

        InputStream is = EnchantedMobs.instance.getResource("languages/en_US.yml");
        if (is != null) {
            try {
                File tempFile = new File(EnchantedMobs.instance.getDataFolder(), "tempMessage.yml");
    ...
    private String getMessage(Player player, String key, String... args) {
        ...
        text = text.replace("{plugin_folder}", String.valueOf(EnchantedMobs.instance.getDataFolder()));
        return text;
    }

    private void saveLanguageFile(String lang, YamlConfiguration config) {
        File file = new File(EnchantedMobs.instance.getDataFolder(), "languages/" + lang + ".yml");
        try {
            config.save(file);
        } catch (IOException e) {
            e.printStackTrace();
